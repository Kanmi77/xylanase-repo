#!/usr/bin/env python3
"""
Merge user-provided experimental metadata into a curated enzyme master table.

This is a universal version of the BRENDA merge step.

It supports:
    - CSV or Excel metadata files
    - user/lab experimental metadata
    - BRENDA-derived metadata
    - literature-curated metadata
    - matching by one or more stable identifiers
    - fill-only mode by default, with optional overwrite

Example for thesis BRENDA metadata:
    python scripts/01_curation/08_merge_experimental_metadata.py \
        --master data/curated/xylanase_master_gh10_gh11_frozen_thesis_with_refseq.csv \
        --metadata-source data/curated/xylanase_master_all_curated_with_brenda.csv \
        --output data/curated/xylanase_master_gh10_gh11_frozen_thesis_with_refseq_experimental_metadata.csv \
        --source-name BRENDA \
        --metadata-column-prefix brenda_ \
        --merge-keys uniprot_accession,primary_id,refseq_acc
"""
