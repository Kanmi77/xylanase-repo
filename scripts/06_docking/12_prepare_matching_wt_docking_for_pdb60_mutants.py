#!/usr/bin/env python3
"""
Prepare matching WT docking for PDB60 mutant docking results.

Purpose:
- Mutant docking alone is not enough.
- We need WT docking for the same structures, using the same receptor PDBQT fallback method
  and the same grid coordinates as the paired mutant docking jobs.

Inputs:
1. Mutant docking manifest:
   results/optionC_original_only/pdb60_foldx_mutations/parsed_ddg_and_docking/mutant_docking_vina_compatible_pdbqt/pdb60_mutant_docking_jobs_manifest_VINA_COMPATIBLE.csv

2. Clean unique PDB representatives:
   results/optionC_original_only/pdb60_foldx_mutations/clean_unique_pdb60_representatives.csv

Outputs:
   results/optionC_original_only/pdb60_foldx_mutations/parsed_ddg_and_docking/wt_matching_docking_vina_compatible_pdbqt/
"""
