#!/usr/bin/env python3
"""
Generate fallback receptor PDBQT files from cleaned mutant PDBs and prepare Vina docking.

Why:
- Open Babel created empty receptor PDBQT files because it failed to kekulize aromatic bonds.
- This script writes simple rigid receptor PDBQT files directly from cleaned PDB ATOM records.
- Charges are set to 0.000 and AutoDock atom types are assigned from elements.
- This is a fallback, used only because Open Babel receptor conversion failed.

Input:
    results/optionC_original_only/pdb60_foldx_mutations/parsed_ddg_and_docking/mutant_docking_cleaned/pdb60_mutant_docking_jobs_manifest_CLEANED.csv

Output:
    results/optionC_original_only/pdb60_foldx_mutations/parsed_ddg_and_docking/mutant_docking_fallback_pdbqt/
"""
